...
tf.app.flags.DEFINE_string("model", "lstm", "RNNCell type, supported: rnn, lstm, gru")
tf.app.flags.DEFINE_boolean("attention", True, "Enable attention mechanism")
...
with tf.Session(config=config) as sess:
    if FLAGS.is_train:
        ...
        name = 'logs/{time}_{model}_{layer}'.format(time=int(time.time()), model=FLAGS.model, layer=FLAGS.layers)
        if FLAGS.attention:
            name += '_attention'
        logger = Logger(name)
        ...
        for epoch in list(range(FLAGS.epoch)):
            ...
            logger.log_scalar('train_loss', loss, epoch)
            logger.log_scalar('train_acc', accuracy, epoch)
            ...
            val_loss, val_accuracy = evaluate(model, sess, data_dev)
            logger.log_scalar('val_loss', val_loss, epoch)
            logger.log_scalar('val_acc', val_accuracy, epoch)
            print("        dev_set, loss %.8f, accuracy [%.8f]" % (val_loss, val_accuracy))
            ...
            print("Best epoch #%d, train_loss %.8f, train_acc [%.8f], val_loss %.8f, val_acc [%.8f]" % (best_epoch, best_train_loss, best_train_acc, best_val_loss, best_val_acc))
            logger.flush()
    else:
        data_dev = load_data(FLAGS.data_dir, 'dev.txt')
