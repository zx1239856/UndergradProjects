...
static uint32_t
sys_get_phy_addr(uint32_t arg[]) {
    uint32_t vaddr = (uint32_t)arg[0];
    if(current != NULL) {
        assert(current->mm != NULL);
        struct vma_struct * vma = find_vma(current->mm, vaddr);
        if(vma != NULL) {
            pte_t *pte = get_pte(current->mm->pgdir, vaddr, 0);
            uint32_t addr = (PPN(*pte) << PTXSHIFT) | PGOFF(vaddr);
            addr = KADDR(addr);
            return addr;
        }
    }
